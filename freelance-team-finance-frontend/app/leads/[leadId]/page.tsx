...
export default function LeadDetailPage() {
  ...
  useEffect(() => {
    (async () => {
      try { const { platforms } = await apiClient.getPlatforms(); setPlatforms(platforms||[]) } catch {}
      fetchLead()
    })()
  ...
  const addFollowUp = async () => {
    if (!fu.date) { alert("Follow-up date/time is required"); return }
    try {
      setBusy(true)
      ...
      await fetchLead()
    } catch (e:any) { alert(e?.message || "Failed to add follow-up") }
    finally { setBusy(false) }
  }
  ...
  return (
    <MainLayout>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <button onClick={deleteLead} className="inline-flex items-center gap-2 px-3 py-2 rounded border text-red-600 hover:bg-red-50">
            <Trash2 className="h-4 w-4"/> Delete
          </button>
          <button onClick={updateLead} className="inline-flex items-center gap-2 px-3 py-2 rounded bg-blue-600 text-white hover:bg-blue-700">
        ...
        <div className="xl:col-span-2 bg-white rounded shadow p-4 space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <input className="border rounded px-3 py-2" placeholder="Client Name *"
              value={edit.clientName} onChange={e=>setEdit((s:any)=>({...s, clientName:e.target.value}))}/>
            <input className="border rounded px-3 py-2" placeholder="Company"
              value={edit.companyName} onChange={e=>setEdit((s:any)=>({...s, companyName:e.target.value}))}/>
            <input className="border rounded px-3 py-2" placeholder="Email"
              value={edit.clientEmail} onChange={e=>setEdit((s:any)=>({...s, clientEmail:e.target.value}))}/>
            <input className="border rounded px-3 py-2" placeholder="Mobile"
              value={edit.clientMobile} onChange={e=>setEdit((s:any)=>({...s, clientMobile:e.target.value}))}/>
            <select className="border rounded px-3 py-2" value={edit.stage}
              onChange={e=>setEdit((s:any)=>({...s, stage:e.target.value}))}>
              {STAGES.map(s=><option key={s} value={s}>{s}</option>)}
            </select>
            <select className="border rounded px-3 py-2" value={edit.priority}
              onChange={e=>setEdit((s:any)=>({...s, priority:e.target.value}))}>
              {PRIORITIES.map(s=><option key={s} value={s}>{s}</option>)}
            </select>
            <input className="border rounded px-3 py-2" placeholder="Estimated Budget" type="number"
              value={edit.estimatedBudget} onChange={e=>setEdit((s:any)=>({...s, estimatedBudget:e.target.value}))}/>
            <select className="border rounded px-3 py-2" value={edit.platform}
            ...
            <input className="border rounded px-3 py-2" placeholder="City"
              value={edit.address?.city||""} onChange={e=>setEdit((s:any)=>({...s, address:{...s.address, city:e.target.value}}))}/>
            <input className="border rounded px-3 py-2" placeholder="Country"
              value={edit.address?.country||""} onChange={e=>setEdit((s:any)=>({...s, address:{...s.address, country:e.target.value}}))}/>
            <input className="border rounded px-3 py-2" placeholder="Tags (comma separated)"
              value={(edit.tags||[]).join(", ")} onChange={e=>setEdit((s:any)=>({...s, tags:e.target.value.split(",").map((x:string)=>x.trim()).filter(Boolean)}))}/>
            <input className="border rounded px-3 py-2" type="datetime-local"
              value={edit.nextFollowUpDate} onChange={e=>setEdit((s:any)=>({...s, nextFollowUpDate:e.target.value}))}/>
          </div>
          <textarea className="border rounded px-3 py-2 w-full" rows={4} placeholder="Project Details / Notes"
            value={edit.projectDetails} onChange={e=>setEdit((s:any)=>({...s, projectDetails:e.target.value}))}/>
          <textarea className="border rounded px-3 py-2 w-full" rows={3} placeholder="Internal Notes"
            value={edit.notes} onChange={e=>setEdit((s:any)=>({...s, notes:e.target.value}))}/>
        </div>
        ...
        <div className="bg-white rounded shadow p-4">
          ...
          <div className="space-y-2 mb-4">
            <input type="datetime-local" className="border rounded px-3 py-2 w-full"
              value={fu.date} onChange={e=>setFu(s=>({...s, date:e.target.value}))}/>
            <input className="border rounded px-3 py-2 w-full" placeholder="Client response"
              value={fu.clientResponse} onChange={e=>setFu(s=>({...s, clientResponse:e.target.value}))}/>
            <textarea className="border rounded px-3 py-2 w-full" rows={2} placeholder="Notes"
              value={fu.notes} onChange={e=>setFu(s=>({...s, notes:e.target.value}))}/>
            <div>
              <label className="block text-xs text-gray-500 mb-1">Set next follow-up date</label>
              <input type="datetime-local" className="border rounded px-3 py-2 w-full"
                value={fu.nextFollowUpDate} onChange={e=>setFu(s=>({...s, nextFollowUpDate:e.target.value}))}/>
            </div>
